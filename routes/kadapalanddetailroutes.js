...
const { KadapaLandDetails } = require("../models/kadapaSchema");
...
const uploadFile = async (req, res) => {
  try {
    ...
    const relativePath = req.file ? `uploads/kadapa/${req.file.filename}` : "";

    const landdetail = new KadapaLandDetails({
      villagename,
      govtland,
      assignedland,
      pattaland,
...
const getLandDetails = async (req, res) => {
  try {
    const landdetails = await KadapaLandDetails.find();
...
const deleteDownload = async (req, res) => {
  try {
    const { id } = req.params;
    const landdetail = await KadapaLandDetails.findByIdAndDelete(id);
...
const updateDownload = async (req, res) => {
  try {
    const { id } = req.params;
    const { villagename, govtland, assignedland, pattaland, total } = req.body;
    const landdetail = await KadapaLandDetails.findById(id);
...
// API Routes
router.post("/kadapalanddetails/upload", upload.single("file"), uploadFile);
router.get("/kadapalanddetails/landdetails", getLandDetails);
router.delete("/kadapalanddetails/landdetails/:id", deleteDownload);
router.put(
  "/kadapalanddetails/landdetails/:id",
  upload.single("file"),
  updateDownload
);
