...
const { KurnoolDownloads } = require("../models/kurnoolSchema");
...
const uploadFile = async (req, res) => {
  try {
    ...
    const relativePath = `uploads/kurnool/${req.file.filename}`; // Store relative path

    const download = new KurnoolDownloads({
      title,
...
const getDownloads = async (req, res) => {
  try {
    const downloads = await KurnoolDownloads.find();
...
const deleteDownload = async (req, res) => {
  try {
    const { id } = req.params;
    const download = await KurnoolDownloads.findByIdAndDelete(id);
...
const updateDownload = async (req, res) => {
  try {
    const { id } = req.params;
    const { title } = req.body;
    const download = await KurnoolDownloads.findById(id);
...
// API Routes
router.post("/kurnooldownloads/upload", upload.single("file"), uploadFile);
router.get("/kurnooldownloads/downloads", getDownloads);
router.delete("/kurnooldownloads/downloads/:id", deleteDownload);
router.put(
  "/kurnooldownloads/downloads/:id",
  upload.single("file"),
  updateDownload
);
